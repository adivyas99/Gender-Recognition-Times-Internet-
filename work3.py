# Work3 -
# Preprocessing the text and then passing it as input-
...
f.write(totalText)'''
df_merged_train_final_copy = df_merged_train_final.copy()
df_merged_train_final_copy['desc'] = df_merged_train_final['desc'].map(PreProcessing)
from sklearn.feature_extraction.text import CountVectorizer
count_vect = CountVectorizer()
X_train_counts = count_vect.fit_transform(df_merged_train_final_copy.desc)
...
clf = MultinomialNB().fit(X_train_tfidf, df_merged_train_final_copy.gender)
...
text_clf = Pipeline([('vect', CountVectorizer()),
                      ...
                      ('clf', MultinomialNB()) ])
text_clf = text_clf.fit(df_merged_train_final_copy.desc, df_merged_train_final_copy.gender)

len(df_merged_train_final_copy[(df_merged_train_final_copy.gender=='F')])
len(df_merged_train_final_copy[(df_merged_train_final_copy.desc=='none')])
...
test_df_merged_final_copy = test_df_merged_final.copy()
test_df_merged_final_copy['desc'] = test_df_merged_final['desc'].map(PreProcessing)


## Prediction
av = pd.Series(['av cool'])
text_clf.predict(av)
predicted = text_clf.predict(test_df_merged_final_copy.desc) #747
...
X_resampled, y_resampled = ros.fit_resample(df_merged_train_final_copy.title, df_merged_train_final_copy.gender)
'''
...
size = test_df_merged_final_copy.shape[0]
for i in range(size):
    print(i)
    #predicted = text_clf.predict(pd.Series([test_df_merged_final_copy.loc[i,'title']]))
    av=str(predicted[i,0])
    usr_id = str(test_df_merged_final_copy.loc[i,'userid'])
    if usr_id not in test_cases.keys():
        test_cases[usr_id]=[]
        test_cases[usr_id].append(av)
    else:
        test_cases[usr_id].append(av)
        #test_cases[str(test_df_merged_final_copy.loc[i,'userid'])].append(predicted[0])
count=0
...
#gk = test_df_merged_final_copy.groupby('userid').get_group('1000021')
#gk.first() 
