...
#text_clf.predict(av)
predicted = text_clf_svm.predict(test_df_merged_final.desc) #747
...
for i in test_cases.keys():
    ...
    cool = list(Counter(test_cases[i]).values())
    #print(cool)
    if '0' in test_cases[i] and '1' not in test_cases[i]:
        test_cases2[str(i)] = ['F']
    elif '0' in test_cases[i]:
        av = list(Counter(test_cases[i]).keys())
        cool = list(Counter(test_cases[i]).values())
        ind = av.index('0')
        if cool[ind]/sum(cool)<=0.3:
            test_cases2[str(i)] = ['F']
        else:
            test_cases2[str(i)] = ['M']
    else:
