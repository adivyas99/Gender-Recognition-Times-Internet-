...
Female_df = df_merged_train.drop_duplicates(subset = 'tags')
...
for i in indices:
    ...
    for j in k:
        keywords_M.append(j)
b_set = list(set(keywords_M))
b_set.remove('')
b_set = set(b_set)
a_set = set(keywords_F)
len(a_set.intersection(b_set))
Male_df_copy1 = Male_df_copy[Male_df_copy['gender']=='F']
df_all_rows = pd.concat([Male_df_copy1, df_merged_train])
...
import os
import numpy as np
from keras.layers import Activation, Conv1D, Dense, Embedding, Flatten, Input, MaxPooling1D,LSTM, Dropout, Bidirectional
...
model.fit(data, labels,validation_split=0.2, batch_size=64, epochs=1)
...
for i in range(size):
    ...
    #str(labelencoder_X.inverse_transform([np.argmax(predicted[i])])[0])
