# taking only important words--NOT Working
...
df_merged_train_final = df_merged_train.loc[:,['title','desc','gender']]
df_merged_train_final = df_merged_train_final.drop_duplicates()
df_merged_train_final = df_merged_train_final.astype(str)
#df_merged_train_final['desc'] = df_merged_train['title']+df_merged_train['desc']
from nltk.corpus import stopwords
from nltk.stem.porter import PorterStemmer
corpus = []
for i in df_merged_train_final['desc']:
...
'''corpus = list(filter(lambda a: a != 'none' , corpus))
corpus = list(filter(lambda a: a != '' , corpus))
corpus = list(filter(lambda a: a != 'nan' , corpus))'''

# Creating the Bag of Words model
from sklearn.feature_extraction.text import CountVectorizer
cv = CountVectorizer(max_features = 35000)
X = cv.fit_transform(corpus).toarray()
y = df_merged_train_final.loc[:, 'gender'].values
